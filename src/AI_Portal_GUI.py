...
# RADLEX look-up table
global RADLEX_lut
RADLEX_lut = list()

# Numbers for RADLEX look-up table
global RADLEX_lut_num
RADLEX_lut_num = list()

# Look-up table for RADLEX playbook
global playbook_lut
playbook_lut = list()

# Numbers for the playbook look-up table
global playbook_lut_num
playbook_lut_num = list()

# List of items to include from the playbook
global playbook_incl
playbook_incl = list()

# List of items to include from the RADLEX list
global RADLEX_incl
RADLEX_incl = list()
...
# Look-up table for RADLEX to LOINC code conversion
global RAD2L_lut_LOINC
RAD2L_lut_LOINC = list()

# Look-up table for RADLEX to RAD code conversion
global RAD2L_lut_RAD
RAD2L_lut_RAD = list()

# Look-up table for RADLEX to RPID conversion
global RAD2L_lut_RPID
RAD2L_lut_RPID = list()

# RADLEX to LOINC dictionary
global RAD2L
RAD2L = dict()

# RPIDs to LOINC dictionary
global RPID2L
RPID2L = dict()

class AI_Portal:
    
    # Currently unused function to get text in a list
    ...
    # Filter procedure results based on current input
    ...
    # Add diagnosis code to RADLEX_incl list 
    def AddDiag(self):
        skip = False
    ...
    # Remove diagnosis code from RADLEX_incl list
    def DelDiag(self):
    ...
    # Filter diagnosis results based on input
    ...
    # Submit query to FHIR server
    def Submit_query(self):
        print('Submitting query...')
        # If no alternate query URL is provided:
        if not self.alt_query_url.get():
            ...
            foundMessage = 'Query found ' + str(len(id_list)) + ' related DICOM studies.'
            print(foundMessage)
            self.num_images_label.configure(text=foundMessage)
            
        # Else, if an alternative url is present
        else:
    ...
    # Retrieve DICOM studies from a system using DICOMweb
    def get_DICOMS(self):
        counter = 0
    ...
    # Thread so that retrieving the DICOMs is not freezing the program
    def Get_DICOM_images(self): 
        thread.Thread(target=self.get_DICOMS).start()
        
    def __init__(self, top=None):
    # This is all setup for the UI. Most is not commented yet.
        ...
        self.get_dicom_button = Button(self.Download_tab, text = 'Get Associated DICOM Images', command=lambda: AI_Portal.Get_DICOM_images(self))
